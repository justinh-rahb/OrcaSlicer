...
unsigned int effective_infill_filament_id(const Layer &layer, const PrintRegionConfig &config, unsigned int filament_id)
{
    ...
    return mixed_mgr.resolve_perimeter(filament_id, num_physical, int(layer.id()),
                                       innermost_perimeter_index, float(layer.print_z), float(layer.height));
}
...
Flow LayerRegion::bridging_flow(FlowRole role, bool thick_bridge) const
{
    ...
    Flow bridge_flow;
    auto nozzle_diameter = float(print_object.print()->config().nozzle_diameter.get_at(
#ifdef ENABLE_FULLSPECTRUM
        this->extruder(role)
#else
        region.extruder(role)
#endif
        - 1));
    if (thick_bridge) {
